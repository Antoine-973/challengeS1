<script setup>
import { ref } from 'vue'
import {getLikes, patchAcceptLikes, patchRejectLikes, getUserInfo} from '../services/likesServices';
...
const getUserConnected = async(id) => {
  ...
  connectedUserSpaId.value = user.spa.split('/')[2];
...
const getLike = async(id) => {
  ...
  filterDataLike.value = data.value.filter(el => el.animalId.spa.split('/')[2] === id);
